...
本目录保存可复制、可实例化的项目模板文件，不作为 Claude Code skill 触发。作为插件安装后，这些模板随插件一起提供，供执行场景 skill 引用。
...
2. 执行场景 skill 应按任务场景、分诊结果、Agent 环境方案和文档初始化方案实例化模板；如果选用 ECC `orch-*`，具体实施流程以 `orch-*` 为主，但执行期记录与交付文档仍按模板维护。
