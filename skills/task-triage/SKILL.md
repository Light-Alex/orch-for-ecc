...
1. 先不要实现。
2. 信息不足时，不给最终分诊；最多问 5 个关键问题，并通过交互模式收集用户确认。
...
7. 用户确认前，不进入下一阶段，也不写入正式 `diagnosis.md`。
8. 非交互输入、沉默、默认选项或模糊的“继续”不构成批准。
9. 优先参考 `${CLAUDE_PLUGIN_ROOT}/orchestration/ecc-capability-map.md` 判断当前任务可能需要的 ECC 能力类别，但本阶段只做能力需求初判，不展开具体 Agent、MCP、workflow 或命令派发方案。
10. 如果可能需要的 ECC 能力类别存在缺失、改名或不适用风险，只记录为 `/agent-env` 需要处理的能力缺口和 Plan B 输入。
11. 如果用户或项目提供 `references/`，按 `${CLAUDE_PLUGIN_ROOT}/orchestration/reference-inputs.md` 将其作为只读参考输入，用于识别目标、范围、非目标、风险、验收候选和冲突；不得把 references 内容自动视为验收标准。
...
## 9. 需要用户确认的问题
1. `<问题>`：为什么重要；用户回答；对执行方案的影响。
2. `<问题>`：为什么重要；用户回答；对执行方案的影响。

交互确认要求：主动通过交互模式逐项确认待确认问题；完成待确认问题后先展示最终草案，再请求用户明确批准写入分诊结果。用户修改、拒绝或未回答关键问题时，不写入正式运行文档。

## 10. 下一步
- 如果用户明确确认全部待确认问题并批准写入：自动写入 `.claude/runs/<date>-<task-slug>/diagnosis.md`，然后可进入 `/agent-env`。
- 如果用户修改：更新分诊草案后重新确认。
- 如果仍不清楚：继续提问，不写入正式运行文档。
```
...
未完成交互确认前只保留草案，不写入正式运行文档。回答问题不等于批准写入；该确认只授权写入 `diagnosis.md`，不授权实现、硬初始化、配置变更、外部访问或项目文件改动。非交互输入、沉默、默认选项或模糊的“继续”不构成批准。
