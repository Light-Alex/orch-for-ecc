...
4. 同一个文档同一阶段只允许一个 owner Agent 写；专家 Agent 可以给建议，但由 owner 合并。
...
11. 优先参考 `orchestration/ecc-capability-map.md` 查询文档、codemap 和质量门禁相关 ECC 能力，并在方案中显式列出推荐调用的 `/ecc:*` 指令。
12. 如果 ECC 文档能力缺失、改名或不适用，按 Plan B 降级为手动文档计划、内建能力或只读文档 Agent，并说明能力缺口。
