...
这些 skill 来自任务分诊、Agent 环境初始化、文档初始化与五类执行场景的编排流程，并已整理为中文入口。当前阶段所有 skill 均为实验性能力，**只应在用户显式输入对应 `/skill-name` 时使用**，不要根据普通自然语言请求自动套用。

本仓库定位为 ECC 插件能力编排器：项目内 skill 负责场景识别、风险收敛、能力组合、输入材料、停止条件和 Plan B；ECC 插件能力优先作为执行器。执行时应显式列出推荐调用的 `/ecc:*` 指令，不要隐藏式链式调用。

运行时以本目录中的 skill 内容为准，不再依赖 `references/agent-workflow-solution.md` 作为执行来源。
...
共享规则、报告模板、ECC 能力映射和 ECC 插件能力基线不属于 skill，不放在本目录中。它们位于项目根目录的 `orchestration/`：
