...
1. 执行场景 skill（定义见 `${CLAUDE_PLUGIN_ROOT}/orchestration/rules.md`）优先把职责匹配的 ECC `orch-*` 指令作为执行器，而不是在项目内重复实现同类流水线。
...
5. 无论是否使用 ECC `orch-*`，执行期记录与交付文档仍按 `${CLAUDE_PLUGIN_ROOT}/templates/workflow-execution-template.md` 的“执行期记录与交付”要求维护。
